...
const emptyUser = { email: '', password: '', role: 'driver' };
...
export default function AdminDashboard() {
  ...
  return <section className="management-page">
    ...
    {tab === 'areas' ? <>
      ...
      <DataTable headers={['Name','Location','Spaces','Workers','Status','Actions']} rows={areas.map(a => [a.name,a.location,`${a.availableSpaces} / ${a.totalSpaces}`,a.assignedWorkers?.map(w=>w.email).join(', ')||'None',a.status,<RowActions onEdit={() => {setEditingArea(a._id);setAreaForm({name:a.name,location:a.location,totalSpaces:a.totalSpaces,availableSpaces:a.availableSpaces,assignedWorkers:a.assignedWorkers?.map(w=>w._id)||[]})}} onDelete={() => remove('parking area',a._id)} />])} />
    </> : <>
      <form className="management-form" onSubmit={submitUser}><h2>{editingUser ? 'Edit user' : 'Add user'}</h2><div className="form-row">
        ...
        <input type="password" minLength="6" placeholder={editingUser ? 'New password (optional)' : 'Password'} value={userForm.password} onChange={e => setUserForm({...userForm,password:e.target.value})} required={!editingUser} />
        <select value={userForm.role} onChange={e => setUserForm({...userForm,role:e.target.value})}><option value="driver">Driver</option><option value="worker">Worker</option><option value="admin">Admin</option></select>
      ...
      <DataTable headers={['Email','Role','Created','Actions']} rows={users.map(u => [u.email,u.role,new Date(u.createdAt).toLocaleDateString(),<RowActions onEdit={() => {setEditingUser(u._id);setUserForm({email:u.email,password:'',role:u.role})}} onDelete={() => remove('user',u._id)} />])} />
    </>}
  </section>;
}
